...
每个模型暴露为同名的 MCP 工具，工具描述会自动以 `(model: <model_id>)` 前缀标明所用的真实模型 id：
...
采用密码登录（默认密码 `password`）。主要功能：仪表盘（请求总数、成功/失败数、成功率、出图数、近 30 天趋势图、按模型成功率统计、失败原因 Top、最近调用记录含耗时与错误）、中英双语（浏览器为中文环境时默认中文，可一键切换并持久化偏好）、深浅主题（首次访问读取系统偏好，之后手动切换并持久化）、模型增删改查（model_id 创建后锁定不可修改，工具名可随时改；描述附填写模板）、全局设置（端口、MCP token、管理密码、清理规则、监听地址）、图片浏览与删除。
...
| `models[].id` | 内部稳定标识（UUID），创建时自动分配，不可修改；统计以此为键，重命名工具名不影响历史数据 |
| `models[].name` | 自定义 alias，用作 MCP 工具名（`^[a-zA-Z][a-zA-Z0-9._-]{0,63}$`），唯一且可随时修改 |
| `models[].model_id` | 真实模型 id，转发上游时使用；**创建后锁定不可修改**；作为仪表盘显示标签（多渠道同模型 id 时以 `model_id · name` 消歧） |
...
**修改工具名后历史统计还在吗？**
在。统计以每个模型创建时分配的内部 UUID（`models[].id`）为键，与工具名、模型 id 解耦；重命名工具名或同模型 id 多渠道都不会丢数据。升级老版本时，原以工具名 / 模型 id 为键的历史统计会在启动时一次性迁移到对应模型的 UUID 上。

**不同渠道同模型 id 怎么统计？**
各自独立计数。两个模型条目可共用同一上游模型 id（不同 `base_url`），仪表盘会以 `model_id · 工具名` 的形式分行展示，互不合并。
